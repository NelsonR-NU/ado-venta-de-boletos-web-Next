...
const TripCard: React.FC<TripCardProps> = ({ title, date, passengers, routes, type }) => {
  ...
  return (
    <Card className="w-[744px] !gap-2 mx-auto !bg-ado-text-white">
      ...
      <div className="flex border-none h-[74px] w-[696px] items-center bg-ado-white p-2 rounded-lg gap-24">
      ...
      <div>
        {type === "ida" && <TripCardIda displayRoutes={displayRoutes} time={time} t={t} />}

        {type === "regreso" && <TripCardRegreso displayRoutes={displayRoutes} time={time} t={t} />}
      </div>
    </Card>
  );
};
