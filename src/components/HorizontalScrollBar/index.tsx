  ...
  return (
    <div className="flex items-center justify-center mt-0 p-0 w-full  mx-auto sm:justify-between sm:gap-[24px]">
      {showArrows && (
        <Button
          buttonStyle="outline"
          className="!p-2 text-gray-600 border !border-ado-gray shadow-green hover:bg-gray-200 !rounded-full hidden md:flex"
          onClick={() => scrollRef.current?.scrollBy({ left: -120, behavior: "smooth" })}
          icon={<Image src={leftArrow} alt="left arrow" className="w-[15px]" />}
        />
      )}
      <div className="relative overflow-hidden  p-0 m-0 w-full   border-0 rounded-md sm:p-4 sm:border sm:border-ado-neutral-light   sm:bg-ado-scroll-background ">
        <div
          ref={scrollRef}
          className="flex gap-2 scrollbar-hidden bg-white   items-center overflow-x-auto  sm:gap-0 sm:bg-inherit"
          ...
          {dates.map(({ day, date, past }, index) => (
            <div
              ...
              onClick={() =>
                handleSelectDate(index, t.raw(`days.${day}`)[1], new Date(date).getDate())
              }
              aria-disabled={past}>
              ...
              <span
                className={`text-sm capitalize ${past ? "text-gray-300" : "text-gray-600"} sm:hidden`}>
                $1,000.00 MXN
              </span>
            </div>
          ))}
        ...
        <div className="flex flex-col gap-[16px] p-[16px]">
          ...
          <div className="flex flex-col justify-center item-center">
            <p className="flex justify-center text-[14px]">
              {t("date_popup.content")}
              <span className="font-bold ml-[5px]">
                {selectedDate?.day} {selectedDate?.date} de{" "}
                {t.raw(`months.${selectedDate?.month || 0}`)}{" "}
              </span>
            </p>
          ...
          <div className="flex justify-between gap-[16px] flex-col w-full sm:flex-row">
            <Button
              ...
              buttonText={t("date_popup.cancel")}
              onClick={handleDateChange}
            />
            <Button
              className={"w-full text-[16px]"}
              buttonText={t("date_popup.continue_btn")}
              onClick={handleDateChange}
            />
          </div>
        </div>
