    ...
    <Modal isOpen={isOpen} onClose={onClose} showCloseIcon={false}>
      <div>
        <div className="flex items-center gap-4">
          <button onClick={onBackToLogin}>
            <Image src={leftChevron} alt="left chevron" className="w-[16px]" />
          </button>
          <h2 className="font-bold text-[18px] text-ado-charcoal py-2">{t("register")}</h2>
        </div>
      </div>
      <form className="space-y-4 my-4">
        ...
        <div className="space-y-2">
          <Checkbox
            label={<span className="text-xs">{t("receive_promotions")}</span>}
          ...
          <Checkbox
            label={
              <span className="text-xs">
                {t("agree_terms")}
                <Link href="#" className="text-ado-royal-purple underline mx-1">
                  {t("terms_conditions")}
                </Link>
                {t("as_well_as_the")}
                <Link href="#" className="text-ado-royal-purple underline mx-1">
