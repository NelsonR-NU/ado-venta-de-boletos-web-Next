...
const PurchasePriceContent: React.FC<PurchasePriceContent> = ({ category, contentCenter, price, className }) => {
    return <div className={className}>
        <span className={`text-sm ${contentCenter ? "min-w-[100px] text-center" : ""}`}>{category}</span>
        <span>{price}</span>
    </div>
}

const PurchaseDetailsCard: React.FC<PruchaseDetailsCardProps> = ({ title, price, items, contentCenter }) => {
    const t = useTranslations("reservationSummary");
    return <div className="bg-ado-white rounded-lg px-6 py-4 w-full">

        <PurchasePriceContent category={title} price={price} className="flex justify-between text-sm font-bold text-ado-charcoal px-2 py-4 border-b border-b-ado-light-blue-gray" />

        <div className="flex flex-col  gap-2 px-2 pt-4">
            {items.map((item, index) => (
                <PurchasePriceContent key={index} contentCenter={contentCenter} category={item.label} price={item.amount} className={"flex justify-between gap-4 text-ado-charcoal"} />
