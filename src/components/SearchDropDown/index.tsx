...
function SearchDropDown({ recentSearches, originTerminals, cardName, onSelect }: SearchDropDownProps) {
    ...
    return (
        <div
            className="absolute left-0 top-full bg-white border border-gray-300 w-[300px] mt-1 rounded-[20px] shadow-lg z-[9999] max-h-120 overflow-auto flex flex-col"
        >
                    ...
                    <div className=' w-full flex flex-col ' >
                        ...
                        <div className=' mt-[8px] bg-[#F6F7FB] w-full h-[100px] p-[8px] flex flex-col justify-between rounded-[8px] ' >
                            {recentSearches.map((item, index) => (
                                <div
                                    key={index}
                                    className="block text-[14px] w-full px-2 py-2 text-ado-text-gray hover:bg-gray-100 cursor-pointer"
                        ...
                        <div className=' mt-[8px] bg-[#F6F7FB] w-full h-[200px] p-[8px] flex flex-col justify-between rounded-[8px] ' >
                            {originTerminals.map((item, index) => (
                                <div className='flex items-center justify-center px-2 ' >
                                    <div
                                        key={index}
                                        className="block text-[14px] w-full py-2 text-ado-text-gray hover:bg-gray-100 cursor-pointer"
                                        onClick={() => onSelect(cardName, item)}
                                    >
                                        {item}
                                    </div>
